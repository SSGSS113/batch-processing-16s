        ...
        try {
            if (Qiime2Service.doClassify(sra)) {
                log.info("Qiime2:doTaxonomy 完成, Sra:{}, 处理线程: {}",
                        sra.getSra().getSraId(), Thread.currentThread().getName());
                outputQueue.put(sra);
                CommonConstant.TAXONOMY_SUCCEED.getAndIncrement();
            } else {
                CommonConstant.TAXONOMY_FAILED.getAndIncrement();
                CommonConstant.FAILED_NUM.getAndIncrement();
                CommonConstant.putData(sra.getSra().getSraId(), type, "Taxonomy 步骤失败");
            }
            ...
            log.info("一共有 {} 条数据，已经处理了 {} 条，其中 {} 成功，共 {} 失败，本步骤失败 {}", CommonConstant.NUM,
                    CommonConstant.TAXONOMY_SUCCEED.get() + CommonConstant.FAILED_NUM.get(),
                    CommonConstant.TAXONOMY_SUCCEED.get(),
                    CommonConstant.FAILED_NUM.get(),
                    CommonConstant.TAXONOMY_FAILED.get());
            log.info("============================================TAXONOMY===========================================");
        }
    }
}
